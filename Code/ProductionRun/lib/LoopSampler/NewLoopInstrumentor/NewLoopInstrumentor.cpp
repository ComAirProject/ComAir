...
STATISTIC(OrigRW, "OrigRW");
STATISTIC(FinalRW, "FinalRW");
...
static cl::opt<bool> bNoOptInst("bNoOptInst", cl::desc("No Opt for Num of Instrument Sites"), cl::Optional, cl::value_desc("bNoOptInst"));

static cl::opt<bool> bNoOptCost("bNoOptCost", cl::desc("No Opt for Merging Cost"), cl::Optional, cl::value_desc("bNoOptCost"));
...
//    std::error_code EC;
//    llvm::raw_fd_ostream outFileIndvarInfo("indvar.info", EC, llvm::sys::fs::F_Text);
//    if (outFileIndvarInfo.has_error()) {
//        errs() << "Cannot open indvar.info\n";
//        return false;
//    }
...
//        OrigRW += inplaceMI.size();
        errs() << "CalleeInplace:" << OrigRW << "\n";
        errs() << "LoopAll:" << inplaceMI.size() << "\n";
        ...
        errs() << "LoopFinalInplace:" << inplaceMI.size() << "\n";
        errs() << "Hoist:" << hoistMI.size() << "\n";
//        DEBUG(dbgs() << "Inplace:\n");
...
//        InstrumentMonitoredInsts(inplaceClonedMI);
    ...
    if (!hoistMI.empty()) {
        Instruction *hoistLoc = pLoop->getLoopPreheader()->getTerminator();
...
//        HoistOrSinkMonitoredInsts(hoistClonedMI, hoistLoc);
        HoistOrSinkMonitoredInsts(hoistMI, hoistLoc);
    }
...
void OptLoopInstrumentor::InlineNumGlobalCost(Loop *pLoop) {
